...
class TeachersController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				teachers.Tea_id LIKE ? OR 
				teachers.Name LIKE ? OR 
				teachers.address LIKE ? OR 
				teachers.phone LIKE ? OR 
				teachers.status LIKE ? OR 
				teachers.DOB LIKE ? OR 
				teachers.POB LIKE ?
			)";
			$search_params = array(
		...
		if(!empty($request->orderby)){
			...
			$db->orderBy("teachers.Tea_id", ORDER_TYPE);
		}
		if($fieldname){
...
// No View Function Generated Because No Field is Defined as the Primary Key on the Database Table
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("Name","address","phone","status","DOB","POB");
